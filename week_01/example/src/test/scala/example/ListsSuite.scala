 ...
 */
 class ListsSuite {
  ...
  @Test def `details why one plus one is not three (0pts)`: Unit = {
    assertEquals(3, 1 + 2) // Fix me, please!
  }
  ...
  // Extra tests
  @Test def `sum of a few negative numbers`: Unit = {
    assert(sum(List(-1,-2,-5)) == -8)
  }

  @Test def `max of a few negative numbers`: Unit = {
    assert(max(List(-1,-2,-5)) == -1)
  }

  @Test def `max throws an exception if empty list`: Unit = {
    val dm = List[Int]()
    try {
      max(dm)
      Assert.fail("No exception has been thrown")
