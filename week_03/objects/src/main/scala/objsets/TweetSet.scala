...
abstract class TweetSet extends TweetSetInterface {
   ...
   * and be implemented in the subclasses?
   */
  def union(that: TweetSet): TweetSet = filterAcc(tweet => this.contains(tweet),that.filterAcc(tweet=>that.contains(tweet),new Empty()))
   ...
   * and be implemented in the subclasses?
   */
  def mostRetweeted: Tweet = mostAcc(new Tweet("placeholder", "placeholder", 0))
  // can you not use qccumulqtor? helper
  def mostAcc(placeholder: Tweet): Tweet
...
class Empty extends TweetSet {
  ...
  def mostAcc(placeholder: Tweet): Tweet = placeholder
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = if (p(elem)) right.filterAcc(p, left.filterAcc(p, acc.incl(elem)))
